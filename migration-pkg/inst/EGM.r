...
obj <- function(x,cash,evplus,pars){
	...
	EV <- EVfun(x,pars$a,evplus,derivs=1)
...
obj.val <- function(x,cash,evplus,pars){
	...
	EV <- EVfun(x,pars$a,evplus,derivs=0)
...
for (ti in (nT-1):1){
	for (yi in 1:nY){
		...
		for (ai in 1:nA){
			...
			con.val[ai,yi,ti] <- s[.(ai,yi,ti)][["cash"]] - (1/dataR$R) * sav.val[ai,yi,ti]
			value.val[ai,yi,ti] <- ufun(con.val[ai,yi,ti],pars=dataR) + dataR$beta*EVfun(sav.val[ai,yi,ti],dataR$a,evtmp,derivs=0)
		}
	}
