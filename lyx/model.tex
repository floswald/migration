...
\usepackage{makecell}
\usepackage{xr}
\externaldocument[R-]{model-online}[https://floswald.github.io/pdf/homeownership-appendix.pdf]
...
Fabien Postel-Vinay and seminar participants at UCL, IFS, Royal Holloway,
Chicago Fed, Boston College, Stockholm School of Economics, DIW Berlin,
Cambridge, Toulouse, Sciences Po, the Bank of England and two anonymous
referees and the Editor for helpful comments and suggestions. I would
like to acknowledge financial support from cemmap under ESRC grant
ES/I034021/1 and ESRC transformative grant ES/M000486/1. I also acknowledge
that the work presented here made use of the IRIDIS High Performance
Computing Facility provided by the Science \& Engineering South (SES)
Centre for Innovation. The Centre is formed by the universities of
Oxford, Southampton, Bristol and University College London.}}
...
contract fairly well. Finally, \cite{bartik2018moving} is a recent
paper which extends \cite{yagan} to consider the influence of the
China trade shock as well as the Fracking boom.
...
\begin{figure}
\begin{centering}
\includegraphics[scale=0.85]{../../../Dropbox/research/mobility/output/data/sipp/raw-moversD2D_v2}
\par\end{centering}
...
\begin{centering}
\includegraphics[scale=0.9]{/Users/florian.oswald/Dropbox/research/mobility/output/data/FHFA/correlogram-detrended}
\par\end{centering}
\end{adjustbox}
...
section, this proceedure needs to replicate the time and age structure
...
all sources and data-cleaning proceedures.} 
...
the aggregate state can be gauged visually in figures \ref{fig:mapping-Y-y}
and \ref{R-fig:mapping-P-p}
...
and by looking at \ref{fig:mapping-Y-y} and \ref{R-fig:mapping-P-p},
...
in this case of a gaussian copula. In particular, $\rho_{s}\in[-1,1]$.}. I report estimates and describe the full procedure in appendix \ref{R-sec:Copula-Estimation}.
...
proceedure seeks to minimize the criterion function 
\begin{equation}
...
derivative of the moment function is not negligible.\footnote{Notice that this proceedure still leads to valid standard errors,
...
with a cyclic coordinate search algorithm, where cycles over the $K$
parameters are defined by
\begin{align*}
\theta^{(2)} & =\arg\min_{\theta_{1}}L(\theta_{1},\theta_{2}^{(1)},\dots,,\theta_{K}^{(1)})\\
\theta^{(3)} & =\arg\min_{\theta_{2}}L(\theta_{1}^{(2)},\theta_{2},\dots,,\theta_{K}^{(2)})\\
 & \vdots\\
\theta^{(K)} & =\arg\min_{\theta_{K}}L(\theta_{1}^{(K-1)},\theta_{2}^{(K-1)},\dots,,\theta_{K}).
\end{align*}

This proceedure is repeated until $\theta$ has converged. Convergence
...
discussion of the fit this provides has been relegated to the online
...
\includegraphics[scale=0.7]{../../../Dropbox/research/mobility/output/model/fit/fit_mobility}\includegraphics[scale=0.7]{../../../Dropbox/research/mobility/output/model/fit/fit_ownership}
