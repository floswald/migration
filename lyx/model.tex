...
Piyapromdee, Richard Blundell, Jose-Victor Rios Rull, Morten Ravn and
Fabien Postel-Vinay for helpful comments and suggestions. The Editor and three referees have helped to greatly improve the paper. I would
...
with a cyclic coordinate search algorithm, where cycle $n$ over the $K$
parameters is defined by
\begin{align*}
\theta^{(n)}_1 & =\arg\min_{\theta_{1}} L(\theta_{1},\theta_{2}^{(n-1)},\dots,,\theta_{K}^{(n-1)})\\
\theta^{(n)}_2 & =\arg\min_{\theta_{2}} L(\theta_{1}^{(n)},\theta_{2},\theta_{3}^{(n-1)},\dots,\theta_{K}^{(n-1)})\\
\theta^{(n)}_3 & =\arg\min_{\theta_{3}} L(\theta_{1}^{(n)},\theta_{2}^{(n)},\theta_{3},\theta_{4}^{(n-1)},\dots,\theta_{K}^{(n-1)})\\
 & \vdots\\
\theta^{(n)}_K & =\arg\min_{\theta_{K}} L(\theta_{1}^{(n)},\theta_{2}^{(n)},\dots,\theta_{K}).
\end{align*}
