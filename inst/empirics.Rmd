...
tmp <- dat[interreg==TRUE,data.frame(expand.grid(to=sort(dat[,unique(region)]),from=sort(dat[,unique(region.l)])),movers=as.numeric(table(region,region.l)))]
...
```{r,message=FALSE,echo=FALSE}
ftable(svytable(~ why.moved + own,subregdes,Ntotal=100))
```
...
```{r,message=FALSE,echo=FALSE}
ftable(svytable(~ empstat + interreg,des,Ntotal=100))
```
...
```{r,message=FALSE,echo=FALSE}
ftable(svytable(~ marstat + interreg,des,Ntotal=100))
```
...
Sample | average wage growth
------ | -------------------
all    | `r dat[incomeFAM>0&incomeFAM.l>0,weighted.mean(log(incomeFAM)-log(incomeFAM.l),weight,na.rm=T)]`
inter state    | `r dat[incomeFAM>0&incomeFAM.l>0&inter==TRUE,weighted.mean(log(incomeFAM)-log(incomeFAM.l),weight,na.rm=T)]`
all region   | `r dat[incomeFAM>0&incomeFAM.l>0&interreg==TRUE,weighted.mean(log(incomeFAM)-log(incomeFAM.l),weight,na.rm=T)]`
...
des = svydesign(ids=~1,weights=~weight,data=dat[incomeFAM>0&incomeFAM.l>0])
m = svyglm(log(incomeFAM) ~ log(incomeFAM.l) + age + I(age^2) + own + wealth + interreg + empstat+marstat + factor(numkids) + State + factor(educ),des)
