...
```bash
# I refer to where you want this on your computer as MIGDIR
# `MIGDIR` is only an example. 
cd MIGDIR  
...
1. Go to where you cloned the package to, e.g. `MIGDIR`.
...
3. Given the non-standard structure of this repo, the above script tries to *sym link* directory `MIGDIR/mig/` into your standard julia package directory at `~/.julia/v0.6` with unix command `ln -s`. This may not work on a windows machine, but [I found this for you to try yourself](https://www.maketecheasier.com/create-symbolic-links-windows10/).
...
The easiest way to deal with dependencies is to directly install from github with this command:
...
This level uses the pre-built input datasets in `MIGDIR/mig/in` to run the structural model. The simplest interface is to use the run file which you can launch on your command line in directory `MIGDIR/mig` as follows:
...
The figures are then created with function [`plot_moment_fit`](https://floswald.github.io/migration/reference/plot_moment_fit.html) in `R` package `migR`.
...
Users may find it helpful to refer to script `MIGDIR/replicate-AWS.sh` to see how I ran this on an [AWS cluster built with cfncluster](https://floswald.github.io/html/cluster.html).
...
With the package loaded as above, you get table 2 with function [`CPS.distance`](https://floswald.github.io/migration/reference/CPS.distance.html)


#### Table 3

Function [`Sipp.SumStats`](https://floswald.github.io/migration/reference/Sipp.SumStats.html).

#### Table 4

Function [`SippProbitMove`](https://floswald.github.io/migration/reference/SippProbitMove.html).

#### Figure 1

Function [`PlotSippMigrationRates`](https://floswald.github.io/migration/reference/PlotSippMigrationRates.html).

#### Figure 2

Function [`correlograms`](https://floswald.github.io/migration/reference/correlograms.html).
...
This is produced by function [`Export.VAR`](https://floswald.github.io/migration/reference/Export.VAR.html)

#### Table 6

This is produced by function [`both_prices_output`](https://floswald.github.io/migration/reference/both_prices_output.html)

#### Income Process

Table and figure C.1 are produced by function [`Export.IncomeProcess`](https://floswald.github.io/migration/reference/Export.IncomeProcess.html)


#### Estimation of Movers' Copula

Figures C.2 and C.3 as well as Table C.2 is produced by [`Sipp.wage_residual_copulas`](https://floswald.github.io/migration/reference/Sipp.wage_residual_copulas.html)

#### Creation of Moments

Function [`Sipp.moments`](https://floswald.github.io/migration/reference/Sipp.moments.html) returns a data.table with data moments.

#### Exporting All Inputs for `julia`

Function [`Export.Julia`](https://floswald.github.io/migration/reference/Export.Julia.html) writes all relevant data to disk in `.rda` format.
...
Once the SIPP database is created locally, we can extract data from it. The function is [`Extract.wrap`](https://floswald.github.io/migration/reference/Extract.wrap.html).

### FHFA and BEA download

Functions [`get_BEA_persincome`](https://floswald.github.io/migration/reference/get_BEA_persincome.html) and [`download.FHFA`](https://floswald.github.io/migration/reference/download.FHFA.html) get the macro data series. Notice that [`get_BEA_persincome`](https://floswald.github.io/migration/reference/get_BEA_persincome.html) depends on external package [EconData](https://github.com/floswald/EconData)
