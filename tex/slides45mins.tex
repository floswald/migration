...
\item \onslide+<2->Analyse empirically how assets and housing choice interact
with migration.
...
\begin{itemize}
\item regional shocks,
\item assets and 
\item housing choices.
\end{itemize} 
\item \onslide+<2->Takes regional house price and income risk seriously (Shocks are not IID!).
\item \onslide+<3->Provides a fully structural implementation of \cite{yagan,yagan2017employment}: \emph{Migratory Insurance}. 
\end{widee}
\end{frame}
...
\begin{widei}
\item Dynamic models of migration over lifecycle: {\footnotesize{}\cite{baum2012understanding}, \cite{kennan2003effect}, \cite{gemici2007family}, \cite{winkler2010effect}\vspace{3mm}}
...
\item Regional Fluctuations: {\footnotesize{}\cite{blanchard1992regional}}{\footnotesize\par}
...
$\tau$ & moving cost type\tabularnewline
$t$ & age\tabularnewline
\hline 
\end{tabular}
\par\end{center}
...
\end{itemize}
\[
u\left(c_{it},h_{it},d,d'\right)=\eta \frac{c_{it}^{1-\gamma}}{1-\gamma}+\xi(s_{it})\times h_{it}-\mathbf{1}\left[d\neq d'\right]\Delta\left(x_{it}\right)+A_{d'}+\varepsilon_{id't}
\]
...
\begin{center}
\textbf{\textcolor{blue}{\LARGE{}Value of the migration option}}{\LARGE\par}
\par\end{center}

\end{frame}
\begin{frame}{Value of the migration option}

\begin{itemize}
\item Migration flows are small, regional shocks are big.
\item Is migration important?
\item Compare two scenarios:

\begin{enumerate}
\item Baseline
\item Cannot move away from West North Central
\end{enumerate}
\end{itemize}
\end{frame}
\begin{frame}[plain]{}
...
\begin{table}
\hspace*{-2pt}\makebox[\linewidth][c]{%
\input{\Dropbox/model/experiments/highMC/mig_value1.tex}
}

\caption{Value of migration option in WNC.}
\end{table}
...
\begin{table}
\hspace*{-2pt}\makebox[\linewidth][c]{%
\input{\Dropbox/model/experiments/highMC/mig_value2.tex}
}
...
\begin{table}
\hspace*{-2pt}\makebox[\linewidth][c]{%
\input{\Dropbox/model/experiments/highMC/mig_value-edit.tex}
}
...
\end{frame}
\begin{frame}{Value of migration by age}

\hspace*{2pt}\makebox[\linewidth][c]{%

\includegraphics[scale=0.85]{\Dropbox/model/experiments/highMC/values_pres}
...
\begin{center}
\textbf{\textcolor{blue}{\LARGE{}Effects of Permanent Regional Shocks}}{\LARGE\par}
\par\end{center}

\end{frame}
%
\begin{frame}{Permanent Shock in West North Central (WNC)}
\begin{itemize}
\item Shocks to regional price and income are correlated.
\begin{itemize}
\item Correlation is measured by Cov Matrix of regional VAR models.
\end{itemize}
\end{itemize}

\pause{}
\begin{itemize}
\item Simulate effects of a permanent $-10\%$ shock to income in $t^{*}=2000$.
\item In region WNC, this translates into a $-6\%$ shock to $p_{WNC,t},t\geq t^{*}$.
\item How do population flows react to that shock? 
\item $-10\%$ is the Kennan and Walker (2011) income shock
\item What's a comparable elasticity here?
\end{itemize}
\end{frame}
%
\begin{frame}{West North Central Shock}

\includegraphics[height=0.9\textheight]{/Users/florian.oswald/Dropbox/research/mobility/output/model/data_repo/out_graphs_jl/outflow}

\end{frame}
%
\begin{frame}{West North Central Shock}
\begin{itemize}
\item 1 \% reduction in regional income $\Rightarrow$ 0.4 \% reduction
of population
\item Kennan and Walker (2011) report elasticity of $\approx0.5$
\item But how do outflows change? Who leaves?
\end{itemize}

\pause{}
\begin{center}
\input{\Dropbox/model/experiments/shockReg8/elasticities.tex}
\par\end{center}
...
\begin{itemize}
\item Constructed a model of migration over the lifecycle capable of dealing
with aggregate shocks.
\item The migration option has a large value.
\item Homeownership: important for internal migration.

\begin{itemize}
\item Affects probability of moving.
\item Affects ability to respond to shocks.
\end{itemize}
\end{itemize}
\end{frame}
%
...
\begin{center}
\textbf{\textcolor{blue}{\LARGE{}Appendix}}{\LARGE\par}
\par\end{center}

\end{frame}

\begin{frame}{Properties: Moving Costs}

\hypertarget{PropMovingCosts}{}

\begin{center}
\begin{table}
\centering{}\input{\Dropbox/model/experiments/moneyMC/moneyMC2.tex}
\end{table}
\par\end{center}
\begin{itemize}
\item Moving cost for young mover in Kennan and Walker (2011): \$384,000

\hyperlink{Properties}{\beamergotobutton{back}}
\end{itemize}
\end{frame}

\begin{frame}{Properties: Decomposing moving costs for owners}

\hypertarget{PropDecomp}{}

\begin{itemize}
\item Why do owners move less?

\begin{itemize}
\item because of utility cost ($\alpha_{3}$)?
\item or because of transaction costs $(\phi)$?
\end{itemize}
\begin{table}
...
\begin{itemize}
\item Transaction cost $\phi$ is important as well (not only utility cost
$\alpha_{3}$).
\end{itemize}
\hyperlink{Properties}{\beamergotobutton{back}}
\end{itemize}
...
\begin{itemize}
\item for each location $d=1,\dots,9$, 

\begin{itemize}
\item compute the value of moving to $d'=1,\dots,9$ 
\item becoming either \textcolor{blue}{owner} or \textcolor{red}{renter}
in $d'$:
\begin{eqnarray*}
{\color{blue}v_{d}^{\text{own}}\left(x_{it},d'\right)} & = & \max_{c}u(c,{\color{blue}1},d,d')+\beta E\overline{v}\left(x_{it+1}\right)\\
{\color{red}v_{d}^{\text{rent}}\left(x_{it},d'\right)} & = & \max_{c}u(c,{\color{red}0},d,d')+\beta E\overline{v}\left(x_{it+1}\right)
\end{eqnarray*}
\item \onslide+<2->\textbf{\textcolor{black}{unconditional value}} of moving
from $d$ to $d'$\textbf{\textcolor{black}{
\begin{eqnarray*}
{\color{black}{\color{green}{\color{black}v_{d}\left(x_{it},d'\right)}}} & = & \max\left({\color{blue}v_{d}^{\text{own}}\left(x_{it},d'\right)},{\color{red}v_{d}^{\text{rent}}\left(x_{it},d'\right)}\right)
\end{eqnarray*}
}}
\end{itemize}
\end{itemize}
...
\[
\text{for all }d=1,\dots,9:\,\,\,\begin{cases}
{\color{green}{\color{black}v_{d}\left(x_{it},1\right)}}=\max & \begin{cases}
{\color{blue}v_{1}^{\text{own}}\left(x_{it},d'\right)}\\
{\color{red}v_{1}^{\text{rent}}\left(x_{it},d'\right)}
\end{cases}\\
{\color{green}{\color{black}v_{d}\left(x_{it},2\right)}}=\max & \begin{cases}
{\color{blue}v_{2}^{\text{own}}\left(x_{it},d'\right)}\\
{\color{red}v_{2}^{\text{rent}}\left(x_{it},d'\right)}
\end{cases}\\
\vdots & \vdots\\
{\color{green}{\color{black}v_{d}\left(x_{it},9\right)}}=\max & \begin{cases}
{\color{blue}v_{9}^{\text{own}}\left(x_{it},d'\right)}\\
{\color{red}v_{9}^{\text{rent}}\left(x_{it},d'\right)}
\end{cases}
\end{cases}
\]

\[
V\left(x_{it},\varepsilon\right)=\max_{d'\in D}\left\{ {\color{green}{\color{black}v\left(x_{it},d'\right)}}+\varepsilon_{id'}\right\} 
\]

\begin{itemize}
\item \onslide+<2->there are 1,410,000 points in the state space $x$
\item \onslide+<2->there are 18 optimization problems at point $x_{it}$:
Solving the model once requires solving 25,380,000 maximization problems.
\end{itemize}
\end{frame}
...
\input{\Dropbox/model/fit/params-pres.tex}\par
\hyperlink{moments}{\beamergotobutton{back}}
\par\end{center}
...
\begin{frame}{Reasons for Moving: SIPP Data\hypertarget{ProbitReasons}{} \hyperlink{SummaryEmpirical}{\beamergotobutton{back}}}

\begin{table}
\begin{centering}
\input{\Dropbox/data/sipp/D2D-probit-edit-slides.tex}
\par\end{centering}
\caption{Probit model, regressing binary moving indicator on household characteristics.
Baseline probability of moving is 1.32\%. Income in \$100,000. }
\end{table}
